        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-20">
            {[
              ...
              <ScrollReveal key={v.title} delay={i * 100}>
                <div className="rounded-2xl border border-border bg-card p-7 hover:border-primary/30 transition-all duration-300">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
              {teamMembers.map((member, i) => (
                <ScrollReveal key={member.name} delay={i * 60}>
                  <div className="group flex flex-col items-center text-center rounded-2xl border border-border bg-card p-5 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/8 transition-all duration-300 hover:-translate-y-1">
                    {/* Avatar */}
                    <div className="relative mb-4">
                      <img
                        src={member.avatar}
                        alt={member.name}
                        className="size-20 rounded-2xl object-cover border-2 border-border group-hover:border-primary/40 transition-colors duration-300"
                      />
                      {/* Online dot */}
              ...
              {/* Placeholder */}
              <ScrollReveal delay={teamMembers.length * 60}>
                <div className="flex flex-col items-center justify-center text-center rounded-2xl border border-dashed border-border bg-card/20 p-5 min-h-[180px]">
          ...
          <div className="mt-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {teamMembers.filter(m => ["Founder", "Co-Founder"].includes(m.role)).map((member, i) => (
                <ScrollReveal key={member.name} delay={i * 120}>
                  <div className="flex items-center gap-5 rounded-2xl border border-border bg-card p-6 hover:border-primary/30 transition-all duration-300 hover:-translate-y-0.5">
                    <img
                      src={member.avatar}
                      alt={member.name}
                      className="size-16 rounded-2xl object-cover border-2 border-border shrink-0"
                    />
                    <div>
                      ...
                      {member.skills && (
                        <div className="flex flex-wrap gap-1.5 mt-3">
                          {member.skills.map(s => (
                            <Badge key={s} variant="secondary" className="rounded-full text-xs bg-muted border-0 text-muted-foreground">{s}</Badge>
                          ))}
                        </div>
                      )}
                    </div>
